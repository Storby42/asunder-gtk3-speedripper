...
void clear_prefs(prefs * p)
{
    ...
    if (p->cddb_email_address != NULL)
        free(p->cddb_email_address);
    p->cddb_email_address = NULL;
}
...
prefs * get_prefs_from_widgets()
{
    ...
    p->music_dir = heap_copy_of(tocopy);
    g_free(tocopy);
    
    p->make_playlist = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(lookup_widget(win_prefs, "make_playlist")));
    
    p->format_music =    heap_copy_of(gtk_entry_get_text(GTK_ENTRY(lookup_widget(win_prefs, "format_music"))));
    p->format_playlist = heap_copy_of(gtk_entry_get_text(GTK_ENTRY(lookup_widget(win_prefs, "format_playlist"))));
    p->format_albumdir = heap_copy_of(gtk_entry_get_text(GTK_ENTRY(lookup_widget(win_prefs, "format_albumdir"))));
    ...
    p->server_name = heap_copy_of(gtk_entry_get_text(GTK_ENTRY(lookup_widget(win_prefs, "server_name"))));
...
void load_prefs(prefs * p)
{
    ...
    if (fd > -1)
    {
        ...
        if (aCharPtr != NULL)
        {
            if (p->cddb_email_address != NULL)
                free(p->cddb_email_address);
            if (0 == strcmp(aCharPtr, "NotSet"))
            	aCharPtr[0] = '\0';	/* 'NotSet' means no email address given */
            p->cddb_email_address = aCharPtr;
        }
