...
GtkWidget * win_upload = NULL;

GtkEntry * glb_album_artist = NULL;
GtkEntry * glb_album_title = NULL;
GtkComboBox * glb_album_category = NULL;
GtkEntry * glb_album_genre = NULL;					// lnr
GtkEntry * glb_album_year = NULL;
GtkToggleButton * glb_single_artist_button = NULL;
GtkWidget * glb_tracklist = NULL;
GtkWidget * glb_upload_button = NULL;

static GtkWidget * pick_disc;
static GtkWidget * statusLbl;
...
int main(int argc, char *argv[])
{
    ...
    glb_album_artist = GTK_ENTRY(lookup_widget(win_main, "album_artist"));
    glb_album_title = GTK_ENTRY(lookup_widget(win_main, "album_title"));
    glb_album_category = GTK_COMBO_BOX(lookup_widget(win_main, ALBUM_CATEGORY_COMBO_BOX));
    glb_album_genre	= GTK_ENTRY(lookup_widget(win_main, "album_genre"));				// lnr
    glb_album_year = GTK_ENTRY(lookup_widget(win_main, "album_year"));
    glb_single_artist_button = GTK_TOGGLE_BUTTON(lookup_widget(win_main, SINGLE_ARTIST_BUTTON));
    glb_tracklist = lookup_widget(win_main, "tracklist");
    glb_upload_button = lookup_widget(win_main, UPLOAD_BUTTON);
    ...
    gtk_tree_view_insert_column_with_attributes(GTK_TREE_VIEW(glb_tracklist), -1, 
                    _("Rip"), renderer, "active", COL_RIPTRACK, NULL);
    GtkTreeViewColumn * col = gtk_tree_view_get_column(GTK_TREE_VIEW(glb_tracklist), COL_RIPTRACK);
    ...
    col = gtk_tree_view_get_column(GTK_TREE_VIEW(glb_tracklist), COL_TRACKNUM);
    ...
    col = gtk_tree_view_get_column(GTK_TREE_VIEW(glb_tracklist), COL_TRACKARTIST);
    ...
    col = gtk_tree_view_get_column(GTK_TREE_VIEW(glb_tracklist), COL_TRACKTITLE);
    gtk_tree_view_column_set_resizable(col, TRUE);
    ...
    GtkTreeViewColumn * concatenatedTitleCol = gtk_tree_view_get_column(
            GTK_TREE_VIEW(glb_tracklist), COL_TRACKARTISTTITLE);
...
GtkListStore* glb_get_tracklist_store()
{
	return GTK_LIST_STORE(gtk_tree_view_get_model(GTK_TREE_VIEW(glb_tracklist)));
}
...
{
	gtk_label_set_markup(GTK_LABEL(statusLbl), _(msg));
}
...
void clear_widgets()
{
    ...
    // clear the textboxes
    gtk_entry_set_text(glb_album_artist, "");
    gtk_entry_set_text(glb_album_title, "");
    gtk_entry_set_text(glb_album_genre, "");				// lnr
    gtk_entry_set_text(glb_album_year, "");
    
    // clear the tracklist
    gtk_tree_view_set_model(GTK_TREE_VIEW(glb_tracklist), NULL);
...
#if 0	/* see http://libcddb.sourceforge.net/tutorial.html: is this not required for gnudb? */
...
void update_tracklist(cddb_disc_t * disc)
{
    ...
    cddb_cat_t disc_category = cddb_disc_get_category(disc);
    cddb_track_t * track;
    bool singleartist;
    char logStr[1024];
    ...
    gtk_tree_view_column_set_visible(gtk_tree_view_get_column(GTK_TREE_VIEW(glb_tracklist),
                COL_TRACKARTISTTITLE),
            FALSE);
    gtk_tree_view_column_set_visible(gtk_tree_view_get_column(GTK_TREE_VIEW(glb_tracklist),
                COL_TRACKTITLE),
    ...
    current_disc_id = cddb_disc_get_discid(disc);

    snprintf(logStr, 1024, "update_tracklist() disk '%08x' '%s' '%s' '%d=%s' '%s' '%d'\n",
        current_disc_id,
		disc_artist,
		disc_title,
		disc_category,
		(disc_category < CDDB_CAT_LAST ? CDDB_CATEGORY[disc_category] : ""),
		disc_genre,
		disc_year);
    debugLog(logStr);
    if (disc_artist != NULL)
    {
        ...
        gtk_entry_set_text(glb_album_artist, disc_artist);
        ...
        gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(lookup_widget(win_main, SINGLE_ARTIST_BUTTON)), singleartist);
    }
    ...
    if (disc_title != NULL)
    {
        ...
        gtk_entry_set_text(glb_album_title, disc_title);
    }

    if (disc_category >= CDDB_CAT_LAST)
    {
    	disc_category = CDDB_CAT_INVALID;
    }
    gtk_combo_box_set_active(glb_album_category, (cddb_cat_t)disc_category);
    ...
    if ( disc_genre )								// lnr
    {
        ...
        gtk_entry_set_text(glb_album_genre, disc_genre );
    }
    else
    {
    	gtk_entry_set_text(glb_album_genre, "Unknown" );
    }
    
    if(disc_year == 0)
    {
        disc_year = EMPTY_YEAR_SHOWN;
    }

    char disc_year_char[5];
    snprintf(disc_year_char, 5, "%d", disc_year);
    gtk_entry_set_text(glb_album_year, disc_year_char );
    
    model = create_model_from_disc(disc);
    gtk_tree_view_set_model(GTK_TREE_VIEW(glb_tracklist), model);
...
void refresh_thread_body(char * cdrom, int force)
{
    ...
    if (check_disc(cdrom) || force)
    {
    	...
    	glb_selected_disc_free();

        current_disc = read_disc(cdrom);
        if (current_disc == NULL)
            return;

        if (current_disc_id != cddb_disc_get_discid(current_disc))
        {
        ...
        GList * disc_matches = lookup_disc(current_disc);
        if ( ! disc_matches)
        {
            glb_selected_disc_clone_from(current_disc); // the last chance to clone
        }
        cddb_disc_destroy(current_disc);
        ...
        if (g_list_length(gbl_disc_matches) > 1)
        {
            ...
            // Note: the selected disc is cloned in on_pick_disc_changed()
        }
        else
        {
        	cddb_disc_t * found_disc = (cddb_disc_t *)g_list_nth_data(gbl_disc_matches, 0);
//            cddb_disc_print(found_disc);
            update_tracklist(found_disc);
