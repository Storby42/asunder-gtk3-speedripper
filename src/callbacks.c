                                        ...
                                        gpointer         user_data)
{
    ...
    {
        sprintf(text, "%d", EMPTY_YEAR_SHOWN);
    }
...
on_pick_disc_changed                   (GtkComboBox     *combobox,
                                        gpointer         user_data)
{
    gint selected_idx = gtk_combo_box_get_active(combobox);
    if (gbl_disc_matches == NULL ||
        selected_idx == -1 ||
        (guint)selected_idx >= g_list_length(gbl_disc_matches))
    {
        return;
    }
	...
	cddb_disc_t * selected_disc = g_list_nth_data(gbl_disc_matches, selected_idx);

	log_disc_info(selected_disc, "selected: ");
    update_tracklist(selected_disc);

    glb_selected_disc_clone_from(selected_disc);
}


void
on_pick_category_changed               (GtkComboBox     *combobox,
                                        gpointer         user_data)
{
	char buf[1024];
	unsigned int max_buf = sizeof(buf) - 1;
    ...
    snprintf(buf, max_buf, "selected category %d = %s\n", selected_idx, CDDB_CATEGORY[selected_idx]);
    debugLog(buf);
}
...
toggle_joined_title_artist                      (void)
{
    if(join_artist_title <= JOIN_OFF)
    {
        ...
        gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(lookup_widget(win_main,
                        SINGLE_ARTIST_BUTTON)), TRUE);
        join_artist_title = JOIN_ON;
    }
    else
    {
        gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(lookup_widget(win_main,
                        SINGLE_ARTIST_BUTTON)), FALSE);
    ...
    GtkTreeViewColumn * titleCol = gtk_tree_view_get_column(GTK_TREE_VIEW(glb_tracklist),
            COL_TRACKTITLE );
...
on_single_artist_toggled               (GtkToggleButton *togglebutton,
                                        gpointer         user_data)
{
    GtkTreeViewColumn * artistCol = gtk_tree_view_get_column(GTK_TREE_VIEW(glb_tracklist),
            COL_TRACKARTIST ); //lnr
